...
2. Find 2-3 actively maintained, canonical, and trending (= what devs are actually reaching for right now, not just what's historically dominant) GitHub repos from the same ecosystem that use patterns that solve the user's problem.
...
4. Analyse the selected repo's pattern. Briefly present the repo's pattern, with links to specific GitHub files. Keep in mind the user is NOT familiar with the internals of these codebases.
